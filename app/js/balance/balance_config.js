...
export const BALANCE_CONFIG = Object.freeze({
  ...
  ELO: {
    ...
    BANDS: [
      { name: 'novato', maxElo: 949, wordFreq: 5, semDist: 0.85 },
      { name: 'normal', maxElo: 1100, wordFreq: 4, semDist: 0.75 },
      { name: 'veterano', maxElo: 1250, wordFreq: 2, semDist: 0.65 },
      { name: 'experto', maxElo: 1500, wordFreq: 1, semDist: 0.50 },
    ],
  },

  // ─── Selección de palabra ──────────────────────────────────
  WORD_SELECTION: {
    // dificultad_objetivo = max(1, 5 - floor((elo - 950) / 100))
    DIFF_BASE: 5,
    DIFF_ELO_OFFSET: 950,
  ...
  // ─── Orden de turno ────────────────────────────────────────
  TURN_ORDER: {
    EARLY_THRESHOLD: 1050,    // ELO ≤ esto → primeras posiciones
    LATE_THRESHOLD: 1200,     // ELO > esto → últimas posiciones
  },
  ...
  SCORING: {
    PARTICIPATION: 75, // Increased from 50
