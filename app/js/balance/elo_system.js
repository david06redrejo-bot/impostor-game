 ...
 *  Players NEVER see their ELO — it only drives word selection,
 *  semantic distance, and turn order.
 * ═══════════════════════════════════════════════════════════════
 */
 ...
 * @returns {{ name: string, difficulty: number, semDist: number }}
 */
export function getDifficultyBand(elo) {
    for (const band of ELO.BANDS) {
        if (elo <= band.maxElo) {
            return { name: band.name, difficulty: band.wordFreq, semDist: band.semDist };
        }
    }
    // Above all bands → highest difficulty
    const last = ELO.BANDS[ELO.BANDS.length - 1];
    return { name: last.name, difficulty: last.wordFreq, semDist: last.semDist };
}
