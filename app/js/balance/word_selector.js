...
const { WORD_SELECTION, ELO } = BALANCE_CONFIG;

/**
 * Calcula la dificultad objetivo de palabra para un ELO dado.
 * Más ELO → palabras más difíciles.
 *
 * @param {number} elo - ELO del impostor
 * @returns {number} Dificultad objetivo (1=baja, 5=muy alta)
 */
export function getTargetWordDifficulty(elo) {
    const raw = WORD_SELECTION.DIFF_BASE -
        Math.floor((elo - WORD_SELECTION.DIFF_ELO_OFFSET) / WORD_SELECTION.DIFF_ELO_DIVISOR);
 ...
 * Más ELO → menos distancia → más difícil camuflarse como impostor.
 *
 * @param {number} elo - ELO del impostor
 * @returns {number} Distancia semántica objetivo (0.45–0.90)
 */
...
export function selectWord(wordPool, impostorElo, recentlyUsed = new Set()) {
    ...
    const scored = available.map(w => ({
        word: w,
        diffDiff: Math.abs(w.difficulty - targetDiff),
    }));
    scored.sort((a, b) => a.diffDiff - b.diffDiff);
...
export function selectSemanticPair(citizenWord, pairs, impostorElo) {
    ...
    // Ordenar pares por cercanía a la distancia objetivo
 ...
 * @property {number} difficulty - 1 (fácil) a 5 (difícil)
 */
